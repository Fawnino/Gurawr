  ...
  execute (message, args) {
    ...
    if (!args.length) {
      ...
      const helpEmbed = new MessageEmbed()
        ...
        .addFields(
          { name: 'Fun', value: '```8ball, coinflip, foodporn, meme, pp, waifu, urbandictionary, anime, kill, kiss, reversetext, hug, bonk, emoji, howgay, diceroll, ricklink, manga```'},
          { name: 'General', value: '```help, invite, ping, advice, gurainfo```'},
          { name: 'Util', value: '```purge, reload, say, stop, add, avatar, wikipedia, itunes, spotifyinfo, welcome, randompassword, server-stats ```'},
          { name: 'Game Info', value: '```genshinconst, genshinchar, genshintalents```'},
          { name: 'Administration', value: '```kick, ban, addrole, removerole, slowmode```'}
        )
